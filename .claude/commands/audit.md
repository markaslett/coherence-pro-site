...
```
echo '{"protocol_version":1,"command":"/audit","status":"[CLEAR/BLOCKED]","emoji":"[:white_check_mark:/:no_entry_sign:]","summary":"[N] requirements — [N] DONE, [N] PARTIAL, [N] MISSING, [N] FAIL","detail_lines":["[blocker details if any]"],"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> /tmp/claude-bridge-summary-${BRIDGE_SESSION}.jsonl
```
